...
Modern attention mechanism kullanan Transformer modeli.
Az veri için optimize edilmiş küçük versiyonu.
"""
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=5000, dropout=0.1):
        ...
        # Pozisyonel encoding matrisi oluştur
...
class TransformerEEG(nn.Module):
    ...
    def __init__(self, input_channels=9, num_classes=3, d_model=64, 
        ...
        # Input projection: feature'ları d_model boyutuna getir
    ...
    def forward(self, x):
        # Input: (batch, seq_len, input_channels)
        
        # Project input to d_model dimensions
        x = self.input_projection(x)  # (batch, seq_len, d_model)
        
        # Scale by sqrt(d_model) - Transformer paper önerisi
...
def train_epoch(model, train_loader, criterion, optimizer):
    ...
    for inputs, labels in train_loader:
        ...
        # Gradient clipping - Transformer'da önemli
...
def plot_training_history(train_losses, val_losses, train_accs, val_accs, save_path):
    ...
    ax1.set_title('Transformer Model - Loss Değişimi', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Transformer Model - Accuracy Değişimi', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(cm, label_names, save_path):
    ...
    plt.title('Transformer Model - Confusion Matrix', fontsize=16, fontweight='bold', pad=20)
...
def save_training_log(log_data, save_path):
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("TRANSFORMER MODEL EĞİTİM LOGU\n")
        f.write("="*70 + "\n\n")
...
def main():
    ...
    print("\n" + "="*70)
    print("TRANSFORMER MODEL EĞİTİMİ")
    print("Attention Mechanism - Az Veri İçin Optimize Edilmiş")
    print("="*70)
    ...
    # Model oluştur - küçük versiyonu (az veri için)
    print("\n" + "="*70)
    ...
    model = TransformerEEG(
        ...
        d_model=64,              # Küçük model boyutu
        nhead=4,                 # Az attention head
        num_layers=2,            # Az katman
        dim_feedforward=256,     # Küçük feedforward
        dropout=0.3              # Yüksek dropout (overfitting önleme)
    ).to(DEVICE)
    
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    
    print(f"✓ Transformer Model oluşturuldu")
    ...
    print("⚠️  Transformer az veriyle zorlanabilir, gözlemleyin!")
    print("="*70)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'transformer_best_model.pth'))
            print(" ✓ BEST", end='')
        
        print()
    
    # Final model kaydet
    torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'transformer_final_model.pth'))
    ...
    plot_training_history(train_losses, val_losses, train_accs, val_accs,
                         os.path.join(SAVE_DIR, 'transformer_training_history.png'))
    
    plot_confusion_matrix(cm, label_names,
                         os.path.join(SAVE_DIR, 'transformer_confusion_matrix.png'))
    ...
    log_data = {
        ...
        'device': str(DEVICE),
        'model_name': 'Transformer (Small - Attention Mechanism)',
    ...
    save_training_log(log_data, os.path.join(SAVE_DIR, 'transformer_training_log.txt'))
    ...
    print(f"  📁 transformer_best_model.pth")
    print(f"  📁 transformer_final_model.pth")
    print(f"  📊 transformer_training_history.png")
    print(f"  📊 transformer_confusion_matrix.png")
    print(f"  📝 transformer_training_log.txt")
    print("="*70 + "\n")

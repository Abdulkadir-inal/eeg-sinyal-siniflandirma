...
# -*- coding: utf-8 -*-
"""
Transformer Model - DATA AUGMENTATION VERSION

Veri çoğaltma teknikleri ile geliştirilmiş versiyon.
Orijinal transformer_model.py'nin tüm özellikleri + data augmentation.
Tüm dosyalar "_augmented" suffix ile kaydedilir.
"""
...
MODEL_NAME = "transformer_augmented"  # ✨ Değişiklik: augmented suffix
BATCH_SIZE = 32
EPOCHS = 50
LEARNING_RATE = 0.001
...
# ============================================================================
# VERİ ÇOĞALTMA FONKSİYONLARI (DATA AUGMENTATION)
# ============================================================================

def add_gaussian_noise(x, noise_level=0.02):
    """Gaussian gürültü ekle - beyin sinyallerindeki doğal varyasyonu simüle eder"""
    noise = torch.randn_like(x) * noise_level
    return x + noise

def time_shift(x, shift_range=5):
    """Zaman kayması - sinyali ileri/geri kaydır"""
    batch_size, seq_len, features = x.shape
    shift = torch.randint(-shift_range, shift_range + 1, (batch_size,))
...
def amplitude_scale(x, scale_range=(0.9, 1.1)):
    """Genlik ölçekleme - EEG güç değişimlerini simüle eder"""
    batch_size = x.shape[0]
    scales = torch.empty(batch_size, 1, 1).uniform_(*scale_range)
    scales = scales.to(x.device)
    return x * scales

def channel_dropout(x, dropout_prob=0.1):
    """Rastgele kanalları (feature) sıfırla - eksik veri simülasyonu"""
    batch_size, seq_len, features = x.shape
    mask = torch.rand(batch_size, 1, features) > dropout_prob
    mask = mask.to(x.device).float()
    return x * mask

def augment_batch(x, augmentation_prob=0.5):
    """
    Batch'e rastgele augmentation uygula
    Her örneğe farklı augmentation kombinasyonları
    """
    augmented = x.clone()
    
    for i in range(x.shape[0]):
        if torch.rand(1).item() < augmentation_prob:
            # Rastgele 1-3 augmentation seç
            aug_count = torch.randint(1, 4, (1,)).item()
            augmentations = torch.randperm(4)[:aug_count]
            
            for aug_type in augmentations:
                if aug_type == 0:
                    augmented[i] = add_gaussian_noise(augmented[i].unsqueeze(0)).squeeze(0)
                elif aug_type == 1:
                    augmented[i] = time_shift(augmented[i].unsqueeze(0)).squeeze(0)
                elif aug_type == 2:
                    augmented[i] = amplitude_scale(augmented[i].unsqueeze(0)).squeeze(0)
                elif aug_type == 3:
                    augmented[i] = channel_dropout(augmented[i].unsqueeze(0)).squeeze(0)
    
    return augmented


# ============================================================================
# MODEL TANIMLARI (Orijinal ile Aynı)
# ============================================================================
...
# ============================================================================
# EĞİTİM FONKSİYONLARI (Augmentation Entegre)
# ============================================================================

def train_epoch_augmented(model, train_loader, criterion, optimizer, device):
    """Eğitim epoch - VERİ ÇOĞALTMA İLE"""
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
    ...
    for inputs, labels in train_loader:
        ...
        # ✨ VERİ ÇOĞALTMA UYGULA
        inputs = augment_batch(inputs, augmentation_prob=0.6)
...
def validate_epoch(model, val_loader, criterion, device):
    """Validation epoch - augmentation YOK"""
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
...
# ============================================================================
# TEST VE GÖRSELLEŞTIRME (Dosya İsimleri Değiştirildi)
# ============================================================================
...
def plot_training_history(history, save_path):
    """Eğitim grafikleri - AUGMENTED suffix ile"""
    ...
    axes[0].set_title('Model Loss (with Data Augmentation)', fontsize=14, fontweight='bold')
    ...
    axes[1].set_title('Model Accuracy (with Data Augmentation)', fontsize=14, fontweight='bold')
...
def plot_confusion_matrix(y_true, y_pred, class_names, save_path):
    """Confusion matrix - AUGMENTED suffix ile"""
    ...
    plt.title('Confusion Matrix (Augmented Data)', fontsize=16, fontweight='bold', pad=20)
...
def mini_prediction_test(model, X_test, y_test, class_names, device, n_samples=10):
    ...
    print("\n" + "="*70)
    print("🎯 MİNİ TAHMİN TESTİ (10 Örnek) - AUGMENTED MODEL")
    print("="*70)
...
def main():
    print("="*70)
    print("TRANSFORMER MODEL EĞİTİMİ - DATA AUGMENTATION VERSION 🔥")
    print("Attention Mechanism + Veri Çoğaltma Teknikleri")
    print("="*70)
    ...
    print(f"✓ Transformer Model oluşturuldu (AUGMENTED VERSION)")
    ...
    print(f"   - Time Shift")
    print(f"   - Amplitude Scale")
    print(f"   - Channel Dropout")
    ...
    print("⚠️  Data Augmentation ile eğitim daha uzun sürebilir!")
    print("="*70)
    ...
    for epoch in range(EPOCHS):
        train_loss, train_acc = train_epoch_augmented(model, train_loader, criterion, optimizer, DEVICE)
    ...
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        f.write("TRANSFORMER MODEL - DATA AUGMENTATION VERSION\n")
        f.write("="*70 + "\n\n")
        f.write(f"Tarih: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Device: {DEVICE}\n")
        f.write(f"Model: Transformer (d_model=64, nhead=4, layers=2)\n")
        f.write(f"Toplam Parametreler: {total_params:,}\n\n")
        f.write(f"Data Augmentation: AKTIF\n")
        f.write(f"- Gaussian Noise\n")
        f.write(f"- Time Shift (±5 steps)\n")
        f.write(f"- Amplitude Scale (0.9-1.1x)\n")
        f.write(f"- Channel Dropout (10%)\n")
        f.write(f"- Augmentation Probability: 60%\n\n")
        f.write(f"Veri:\n")
